...
public class MinKBitFlips {
    ...
    public static int minKBitFlips(int[] A, int k) {
        int N = A.length;
        int[] hint = new int[N]; // 数组记录每一个元素是否需要翻转的状态
        ...
        // 需要翻转子数组形如：A[i], A[i+1], ..., A[i+K-1]
        // 可以在此位置，置为翻转状态，并且在位置 i + k 上设置一个提醒，表示那里也需要置翻转状态
        for (int i = 0; i < N; i++) {
            flip ^= hint[i];
            // 我们是否必须翻转从此开始的子数组, 即 flip 0 则开始，
            if (A[i] == flip){
                ans++;  // 翻转子数组 A[i] 至 A[i+K-1] 翻转次数加一
    ...
    public static void main(String[] args) {
        ...
        System.out.println(minKBitFlips(arr, 3));
